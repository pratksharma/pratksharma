<a href="https://pratk.in" target="_blank" rel="noopener noreferrer">
  <picture width="100%">
    <source media="(prefers-color-scheme: dark)" srcset="./assets/profile-banner-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="./assets/profile-banner-light.svg">
    <img alt="Visit pratk.in" src="./profile-banner-light.svg">
  </picture>
</a>

<br>
<br>

<table>
  <tr>
    <td width="50%" valign="top">
      <sub>01 / READING</sub><br /><br />
      <a href="https://github.com/pratksharma/ReadWiki"><strong>ReadWiki</strong></a><br />
      Modern Wikipedia reader with offline support and distraction-free reading experience.<br /><br />
      <sub>React Native · Expo · TypeScript</sub>
    </td>
    <td width="50%" valign="top">
      <sub>02 / COMMUNITY</sub><br /><br />
      <a href="https://github.com/pratksharma/PariSar"><strong>PariSar</strong></a><br />
      Comprehensive residential society management with visitors, complaints, and amenities.<br /><br />
      <sub>React Native · Express · MongoDB</sub>
    </td>
  </tr>
</table>

<!-- <p align="center" style="white-space: nowrap;">
  <a href="https://pratk.in"><img src="https://img.shields.io/badge/pratk.in-000000?style=for-the-badge"></a>&nbsp;&nbsp;<a href="https://x.com/pratk_sharma"><img src="https://img.shields.io/badge/-000000?style=for-the-badge&logo=x&logoColor=white"></a>&nbsp;&nbsp;<a href="https://linkedin.com/in/pratksharma"><img src="https://img.shields.io/badge/LinkedIn-000000?style=for-the-badge&logo=linkedin&logoColor=white"></a>
</p> -->

<div align="center">
  <br />
  <a href="https://pratk.in">Website</a>&nbsp;&nbsp;·&nbsp;&nbsp;<a href="https://x.com/pratk_sharma">X</a>&nbsp;&nbsp;·&nbsp;&nbsp;<a href="https://linkedin.com/in/pratksharma">LinkedIn</a>
</div>
